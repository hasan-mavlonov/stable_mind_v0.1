...
MindForm runs entirely on Google's **Gemma** open models, served through
the **Gemini API (Google AI Studio)**.

- All LLM access is centralized in [`core/llm.py`](core/llm.py) using the
  official `google-genai` SDK.
- The **Perception Engine** ([`core/perception.py`](core/perception.py))
  asks a Gemma model to extract structured, ontology-constrained
  perceptions as JSON.
- The **Talk** interface ([`talk.py`](talk.py)) uses Gemma to classify the
  asked entity and to rephrase existing beliefs in the persona's tone.
- The default model is `gemma-3-27b-it`; override per engine with
...
MINDFORM_MODEL=gemma-3-27b-it
TALK_MODEL=gemma-3-27b-it
PERCEPTION_MODEL=gemma-3-27b-it
