...
class Agent:
    def __init__(self, root_dir: str, model: str = "gpt-4.1-mini"):
        ...
        self.reflection = ReflectionEngine(root_dir=root_dir)
    ...
    # ----------------------------
    def _update_dynamic_now(self, dyn: Dict[str, Any], user_message: str, entities: List[Dict[str, Any]],
                            turn: int) -> None:
        """
        ...
        # Keyword cues (small, stable nudges)
        emotion_hits = {
            "joy": ["happy", "glad", "joy", "great", "wonderful", "amazing", "excited", "love", "enjoy"],
            "trust": ["trust", "safe", "secure", "supported", "reliable"],
            "fear": ["afraid", "scared", "fear", "anxious", "worried", "panic"],
            "surprise": ["surprised", "shock", "shocked", "unexpected"],
            "sadness": ["sad", "down", "depressed", "lonely", "cry", "hurt"],
            "disgust": ["disgust", "gross", "nasty", "repuls"],
            "anger": ["angry", "mad", "furious", "rage", "annoyed", "yell", "shout"],
            "anticipation": ["hope", "looking forward", "cant wait", "can't wait", "anticipat"],
        }
        ...
        # Influence from perceptions (small)
        for e in entities or []:
            if not isinstance(e, dict):
                continue
            et = e.get("entity_type")
    ...
    def step(self, user_message: str, session_id: str = "default") -> Dict[str, Any]:
        ...
        perception_result = self.perception.analyze(user_text=user_message, session_id=session_id)
        ...
        if int(counters["turns_since_last_rumination"]) >= window:
            ...
            if buffered:
                self.consolidation.run(persona=persona, perceptions=buffered, turn=turn)
            ...
            # Reflection happens after consolidation, only at rumination time
            did_reflect = bool(self.reflection.run(persona=persona, turn=turn))
        ...
        return {
            ...
            "trait_vector": persona.get("stable", {}).get("trait_vector", {}),
